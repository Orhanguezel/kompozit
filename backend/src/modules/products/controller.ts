...
/**
 * GET /products
 * ?category_id=&sub_category_id=&category_slug=&sub_category_slug=&tag=&is_active=&q=&limit=&offset=&sort=&order=&slug=&locale=&item_type=
 *
...
export const listProducts: RouteHandler = async (req, reply) => {
  ...
  if (q.category_slug) conds.push(eq(categoryI18n.slug, q.category_slug));
  if (q.sub_category_slug) conds.push(eq(subCategoryI18n.slug, q.sub_category_slug));
  ...
  if (q.tag?.trim()) {
    conds.push(sql`JSON_SEARCH(${productI18n.tags}, 'one', ${q.tag.trim()}) IS NOT NULL`);
  }
