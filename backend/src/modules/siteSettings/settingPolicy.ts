...
import {
  type JsonLike,
  homeHeroSettingSchema,
  homeMetricsSettingSchema,
  productsB2bSettingSchema,
  homeValuePropsSettingSchema,
} from './validation';
...
export function normalizeValueByKey(key: string, value: JsonLike): JsonLike {
  ...
  if (k === 'home.hero') {
    return homeHeroSettingSchema.parse(value) as unknown as JsonLike;
  }

  if (k === 'home.metrics') {
    return homeMetricsSettingSchema.parse(value) as unknown as JsonLike;
  }

  if (k === 'home.value_props') {
    return homeValuePropsSettingSchema.parse(value) as unknown as JsonLike;
  }

  if (k === 'products.b2b') {
    return productsB2bSettingSchema.parse(value) as unknown as JsonLike;
  }
