...
async function getSiteName(): Promise<string> {
  ...
  const fallback = toLine(titleRow?.value) ?? 'MOE Kompozit';
...
async function getTransporter(): Promise<{ transporter: Transporter; from: string }> {
  ...
  const fromEmail = cfg.fromEmail || cfg.username || 'no-reply@moekompozit.local';
  const from = cfg.fromName ? `${cfg.fromName} <${fromEmail}>` : fromEmail;

  return { transporter: cachedTransporter, from };
}

export async function sendKompozitMail(input: SendMailInput) {
...
export async function sendKompozitWelcomeMail(input: WelcomeMailContext) {
  ...
  await sendKompozitMail({ to: input.to, subject, html });
}

export async function sendKompozitPasswordChangedMail(input: PasswordChangedMailContext) {
  const siteName = input.site_name?.trim() || (await getSiteName());
  const userName = input.user_name?.trim() || input.to.split('@')[0] || 'Kullanici';
...
export async function sendKompozitContactAdminMail(input: ContactMailContext) {
  ...
  await sendKompozitMail({ to: adminTo, subject, html });
}

export async function sendKompozitContactAutoReplyMail(input: ContactMailContext) {
  const siteName = await getSiteName();
  ...
  await sendKompozitMail({ to: input.email, subject, html });
}
...
export async function sendKompozitOfferCustomerMail(input: OfferMailContext) {
  const siteName = await getSiteName();
...
export async function sendKompozitOfferAdminMail(input: OfferMailContext, to: string) {
  const siteName = await getSiteName();
  ...
  await sendKompozitMail({ to, subject, html });
}

export async function sendKompozitOfferRequestAdminMail(input: OfferMailContext, to: string) {
  const siteName = await getSiteName();
