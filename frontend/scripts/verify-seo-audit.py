...
paths=['/'+loc+path for loc in ['tr','en'] for path in ['', '/products','/solutions','/references','/gallery','/blog','/about','/contact','/offer']]
def fetch(path):
...
def verify(path):
 ...
 if path in ['/tr','/en']:
  for target in ['solutions','references','gallery','blog']:assert path+'/'+target in p.links,(path,target)
...
assert len({r['og_image'] for r in rows})==18
llms,_=fetch('/llms.txt');links=re.findall(r'\]\((https?://[^)]+)\)',llms.decode());assert len(links)>=18,len(links)
