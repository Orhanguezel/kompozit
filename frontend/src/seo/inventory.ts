...
export async function withRouteMetadata(metadata: Metadata, locale: string, path: string): Promise<Metadata> {
  ...
  return { ...metadata, alternates, openGraph: { ...metadata.openGraph, url: pageUrl(locale, path), locale: locale === 'tr' ? 'tr_TR' : 'en_GB' } };
}
...
/** Resolve aliases using stable content IDs, never by copying a foreign slug. */
export async function requireLocalizedSlug(section: string, slug: string, locale: string): Promise<void> {
  const current = await getSeoRecords(section, locale);
  if (current.some(row => row.slug === slug)) return;
  for (const other of AVAILABLE_LOCALES) {
    if (other === locale) continue;
    const original = (await getSeoRecords(section, other)).find(row => row.slug === slug);
    const translated = original && current.find(row => row.id === original.id);
    if (translated) permanentRedirect(`/${locale}/${section}/${translated.slug}`);
  }
  notFound();
}
