...
export const GeneralSettingsTab: React.FC<GeneralSettingsTabProps> = ({ locale, settingPrefix }) => {
  ...
  const busy =
    qGlobal.isLoading || qLocale.isLoading || qGlobal.isFetching || qLocale.isFetching || isSaving || isDeleting;
    ...
    <div className="space-y-6">
        ...
        <CardHeader className="gap-2">
          <div className="flex flex-col gap-2 lg:flex-row lg:items-start lg:justify-between">
            <div className="space-y-1">
              ...
              <CardDescription>{t("admin.siteSettings.general.description", { locale })}</CardDescription>
            </div>
        ...
        <CardContent className="space-y-4">
          ...
          {busy ? (
            <div className="flex items-center gap-2">
              ...
              <span className="text-muted-foreground text-xs">{t("admin.siteSettings.general.loadingNote")}</span>
            </div>
          ) : null}
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
                ...
                {groups.length ? (
                    ...
                    return (
                      <TableRow key={`group_${g.key}`}>
                        <TableCell className="align-top">
                          <div className="wrap-break-word font-medium font-mono text-sm">{g.key}</div>
                          <div className="mt-2 flex flex-wrap gap-2">
                            <Badge variant="outline" className="text-xs">
                              {hasGlobal
                                ? t("admin.siteSettings.general.globalExists")
                                : t("admin.siteSettings.general.globalNotExists")}
                            </Badge>
                            <Badge variant="outline" className="text-xs">
                              {hasLocal
                                ? t("admin.siteSettings.general.localeExists", { locale })
                                : t("admin.siteSettings.general.localeNotExists", { locale })}
                            </Badge>
                          </div>
                        </TableCell>

                        <TableCell className="align-top">{sourceBadge(g.effectiveSource)}</TableCell>

                        <TableCell className="align-top">
                          <div className="wrap-break-word max-w-md overflow-hidden text-muted-foreground text-xs">
                  ...
                  <TableRow>
                    <TableCell colSpan={4} className="py-10 text-center text-muted-foreground">
                      ...
                      <div className="mt-2">
                        <Button type="button" variant="outline" size="sm" onClick={upsertEmptyGlobals} disabled={busy}>
                          {t("admin.siteSettings.general.globalBootstrap")}
                        </Button>
                      </div>
                    </TableCell>
          ...
          <div className="text-muted-foreground text-xs">{t("admin.siteSettings.general.deleteTooltip")}</div>
        </CardContent>
      </Card>
    </div>
  );
