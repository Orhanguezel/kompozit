...
import {
  isKompozitHomeSettingKey,
  parseKompozitHomeSettingRawText,
} from '@/lib/kompozit-home-settings-admin';
        ...
        if (parsed.status === 'invalid') {
          const detail =
            parsed.message === 'root_must_be_object'
              ...
              : parsed.message;
          toast.error(
            t('admin.siteSettings.form.kompozitHome.parseError', { message: detail }),
          );
          return;
        }
      }
          ...
          <div className="space-y-3">
              ...
              <div className="space-y-2 rounded-md border bg-muted/30 p-3">
                ...
                <ScrollArea className="max-h-56 w-full rounded-md border bg-background">
                  <pre className="p-3 text-xs leading-relaxed">
                    {JSON.stringify(kompozitHomeParse.value, null, 2)}
                  </pre>
                </ScrollArea>
              </div>
            ) : null}
            ...
            {kompozitHomeParse?.status === 'empty' && isKompozitHomeSettingKey(settingKey) ? (
              <p className="text-xs text-muted-foreground">
                {t('admin.siteSettings.form.kompozitHome.emptyHint')}
              </p>
            ) : null}
