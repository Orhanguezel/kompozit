  ...
  return (
    <div className="space-y-3">
      ...
      <AdminJsonEditor
        ...
        onChange={(next) => setValue(next)}
        disabled={disabled}
        helperText="Required keys: badge, title, subtitle, primary/secondary CTA labels and hrefs, workflow labels."
        height={360}
      />
    </div>
  );
  ...
  return (
    <div className="space-y-3">
      ...
      <AdminJsonEditor
        ...
        onChange={(next) => setValue(next)}
        disabled={disabled}
        helperText="Expected shape: items[3], workflowSteps[3], stats[2]. Backend validation rejects incomplete arrays."
        height={420}
      />
    </div>
  );
...
const HomeValuePropsStructuredRenderer: React.FC<StructuredRenderProps> = ({ value, setValue, disabled }) => {
  ...
  return (
    <div className="space-y-3">
      ...
      <AdminJsonEditor
        label="home.value_props"
        value={normalized}
        onChange={(next) => setValue(next)}
        disabled={disabled}
        helperText="Expected shape: sectionLabel, title, subtitle, and at least 4 items with key/title/description."
        height={400}
      />
    </div>
  );
