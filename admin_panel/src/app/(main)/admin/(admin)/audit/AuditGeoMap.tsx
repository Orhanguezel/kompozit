...
export const AuditGeoMap: React.FC<Props> = ({ items, loading }) => {
  ...
  const getCountryName = useCallback((code: string): string => t(`map.countries.${code}`, undefined, code), [t]);

  const tooltipPos = tooltip ? clampTooltip(tooltip.x, tooltip.y) : null;

  return (
    <div className="space-y-4">
      <div className="relative min-h-[400px] overflow-hidden rounded-lg border bg-card">
        {loading && (
          <div className="absolute inset-0 z-10 flex items-center justify-center bg-background/60">
            <span className="animate-pulse text-muted-foreground text-sm">{t("map.loading")}</span>
          </div>
        )}
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="rounded-lg border bg-card p-4">
          ...
          <div className="flex items-center gap-1">
            {COLOR_STEPS.map((c) => (
              <div key={c} className="h-4 flex-1 first:rounded-l last:rounded-r" style={{ backgroundColor: c }} />
            ))}
          </div>
