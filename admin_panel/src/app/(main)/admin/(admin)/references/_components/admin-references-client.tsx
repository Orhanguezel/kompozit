...
function toPublishedFilter(value: string): PublishedFilter {
...
export default function AdminReferencesClient() {
  ...
  React.useEffect(() => {
    ...
    setFilters((prev) => {
      ...
      return { ...prev, locale: getFirstLocaleValue(localeOptions) };
    });
  ...
  const listQ = useListReferencesAdminQuery(queryParams, {
  ...
  async function onTogglePublished(item: ReferenceDto, next: boolean) {
    ...
    try {
      await updateReference({ id, patch: { is_published: next ? 1 : 0 } }).unwrap();
      toast.success(t("admin.references.list.statusUpdated"));
      listQ.refetch();
    } catch (err) {
      toast.error(getErrMessage(err, t("admin.references.list.statusUpdateError")));
    }
  }
  ...
  async function onToggleFeatured(item: ReferenceDto, next: boolean) {
    ...
    try {
      await updateReference({ id, patch: { is_featured: next ? 1 : 0 } }).unwrap();
  ...
  async function onDelete(item: ReferenceDto) {
    ...
    try {
      ...
      toast.error(getErrMessage(err, t("admin.references.list.deleteError")));
    }
  }
  ...
  const resolveImg = (item: ReferenceDto) => item.featured_image_url_resolved || item.featured_image || "";
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(v) => setFilters((p) => ({ ...p, publishedFilter: toPublishedFilter(v) }))}
                disabled={busy}
              >
                <SelectTrigger>
          ...
          <div className="rounded-md border md:hidden">
            <div className="divide-y">
              {items.map((item) => {
                ...
                const slug = String(item?.slug ?? "").trim() || "—";
                const isPublished = isTruthyBoolLike(item.is_published);
                const _isFeatured = isTruthyBoolLike(item.is_featured);
