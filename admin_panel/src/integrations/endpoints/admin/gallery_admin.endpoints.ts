...
export const galleryAdminApi = baseApi.injectEndpoints({
  endpoints: (build) => ({
    /* -------------------- LIST (admin) -------------------- */
    listGalleriesAdmin: build.query<GalleryListResponse, GalleryListQueryParams | undefined>({
    ...
    /* -------------------- GET BY ID (admin) ---------------- */
    getGalleryAdmin: build.query<GalleryDto, { id: string | number; locale?: string } | string | number>({
      query: (arg) => {
    ...
    /* -------------------- UPDATE (admin) ------------------- */
    updateGalleryAdmin: build.mutation<GalleryDto, { id: string; patch: GalleryUpsertPayload }>({
      query: ({ id, patch }) => ({
    ...
    listGalleryImagesAdmin: build.query<GalleryImageDto[], { galleryId: string; locale?: string }>({
      ...
      providesTags: (_res, _err, arg) => [{ type: "AdminGalleryImages", id: arg.galleryId }],
    }),
    ...
    >({
      ...
      invalidatesTags: (_res, _err, arg) => [{ type: "AdminGalleryImages", id: arg.galleryId }],
    }),
    ...
    /* -------------------- IMAGE REORDER (admin) ------------ */
    reorderGalleryImagesAdmin: build.mutation<void, { galleryId: string; ids: string[] }>({
      query: ({ galleryId, ids }) => ({
