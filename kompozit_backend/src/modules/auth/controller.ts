...
import {
  sendKompozitPasswordChangedMail,
  sendKompozitWelcomeMail,
} from "@/core/kompozit-mail";
import {
  notifications,
...
export function makeAuthController(app: FastifyInstance) {
  ...
  return {
    ...
    signup: async (req: FastifyRequest, reply: FastifyReply) => {
      ...
      void sendKompozitWelcomeMail({
        to: email,
        user_name: userNameForMail,
        user_email: email,
        site_name: "MOE Kompozit",
      }).catch((err) => {
    ...
    passwordResetConfirm: async (
      ...
      if (targetEmail) {
        ...
        void sendKompozitPasswordChangedMail({
          to: targetEmail,
